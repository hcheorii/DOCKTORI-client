...
export const useAuth = () => {
    ...
    const userSignup = useMutation({
        mutationFn: (data: SignupInfo) => signup(data),
        onSuccess: () => {
    ...
    const userLogout = useMutation({
        mutationFn: (token: string) => logout({ accessToken: token }),
        onSuccess: () => {
            storeLogout();
    ...
    const userChangePassword = useMutation({
        mutationFn: (data: UserChangePassword) => changepassword(data),
        onSuccess: () => {
    ...
    const userWithdrawal = useMutation({
        mutationFn: (token: string) => withdrawal({ accessToken: token }),
        onSuccess: () => {
            localStorage.removeItem("token");
    ...
    return {
        userSignup: userSignup.mutate,
        userLogin: userLogin.mutate,
        userChangePassword: userChangePassword.mutate,
        userWithdrawal: userWithdrawal.mutate,
        userLogout: userLogout.mutate,
